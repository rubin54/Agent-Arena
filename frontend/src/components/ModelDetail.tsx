      ...
      footer={
        <>
          ...
          <Button variant={selected ? 'secondary' : 'primary'} onClick={onToggle}>
            {selected ? 'Aus Auswahl entfernen' : 'Zur Auswahl hinzufügen'}
          </Button>
        </>
      }
    >
      <div className="space-y-5">
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-1">
            ...
            {model.is_moderated && <Badge tone="amber">moderiert</Badge>}
          </div>
        </div>
        ...
        {model.description && (
          <div>
            <h3 className="label">Beschreibung</h3>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <div>
            <h3 className="label">Preise (pro 1M Token, außer Request)</h3>
            <div className="rounded-lg border border-ink-700 bg-ink-900 px-3 py-1">
              ...
              <PriceRow label="Cache lesen" value={model.price_cache_read} />
              <PriceRow label="Cache schreiben" value={model.price_cache_write} />
              <PriceRow label="Interner Reasoning-Token" value={model.price_internal_reasoning} />
              <PriceRow label="Bild" value={model.price_image} />
              <PriceRow label="Web-Suche" value={model.price_web_search} />
              {model.price_request !== null && model.price_request !== 0 && (
                <div className="flex items-baseline justify-between py-1.5 text-xs">
                  <span className="text-ink-400">Pro Request</span>
          ...
          <div className="space-y-4">
            <div>
              <h3 className="label">Modalitäten</h3>
            ...
            <div>
              <h3 className="label">Unterstützte Parameter</h3>
              <div className="flex flex-wrap gap-1">
                {model.supported_parameters.length ? (
                  ...
                  <span className="text-xs text-ink-500">keine Angaben</span>
                )}
              </div>
            </div>
          </div>
