      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-ink-500">
            ...
            {data?.fetched_at && ` · updated ${formatRelative(data.fetched_at)}`}
            {data?.stale && ' · stale'}
          </p>
        </div>
        <Button
          ...
          title="Reload the catalog from OpenRouter"
        >
      ...
      <div className="card space-y-3 p-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select
            className="min-w-44"
            value={filters.sort}
            options={SORT_OPTIONS.map((s) => ({ label: `Sort: ${s.label}`, value: s.value }))}
        ...
        <EmptyState
          ...
          hint={
            models.length
              ...
              : 'The catalog is empty. Reload it with "Refresh catalog".'
          }
        />
      ) : (
      ...
      {selection.selected.length > 0 && (
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-ink-700 bg-ink-900/95 backdrop-blur">
          <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-3 px-6 py-3">
            <Badge tone="accent">{selection.selected.length} selected</Badge>
            <div className="flex min-w-0 flex-1 flex-wrap gap-1 overflow-hidden">
              ...
              {selection.selected.length > 6 && (
                <span className="self-center text-[11px] text-ink-500">
                  +{selection.selected.length - 6} more
                </span>
              )}
            </div>
